...
class Model
{
	public:
		...
		public:
			...
			virtual volpe::Material* getMaterial(const std::string& nodeName, int subMeshIndex, const std::string& name) const 
			{ 
				return nullptr; 
			}
		};
		...
		void OverrideMaterial(volpe::Material* pMat);
	...
	protected:
		...
		struct Mesh
		{
			...
			volpe::VertexDeclaration* m_pDecl;
		...
		struct MeshNode
		{
			...
			volpe::Material* m_pMat = nullptr;
			volpe::Material* m_pMatOverride = nullptr;
			std::string m_name;
		...
		volpe::Material* _createMaterialFromAssimpMaterial(const aiMaterial* pAssimpMat);
		...
		std::map<int, volpe::Material*> m_createdMaterials;
		//-------------------------------------------------------------------------
};
