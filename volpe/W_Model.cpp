...
{
    volpe::CT_Invalid,   //EPODDataNone,
	volpe::CT_Float,		//EPODDataFloat,
	volpe::CT_Int,		//EPODDataInt,
	volpe::CT_UShort,	//EPODDataUnsignedShort,
	volpe::CT_UByte4,	//EPODDataRGBA,
	volpe::CT_UByte4,	//EPODDataARGB,
	volpe::CT_UByte4,	//EPODDataD3DCOLOR,
	volpe::CT_UByte4,	//EPODDataUBYTE4,
	volpe::CT_Invalid,	//EPODDataDEC3N,
	volpe::CT_Invalid,	//EPODDataFixed16_16,
	volpe::CT_UByte,		//EPODDataUnsignedByte,
	volpe::CT_Short,		//EPODDataShort,
	volpe::CT_ShortNorm,	//EPODDataShortNorm,
	volpe::CT_Byte,		//EPODDataByte,
	volpe::CT_ByteNorm,	//EPODDataByteNorm,
	volpe::CT_UByteNorm, //EPODDataUnsignedByteNorm,
	volpe::CT_UShortNorm, //EPODDataUnsignedShortNorm,
	volpe::CT_UInt,		//EPODDataUnsignedInt
};
...
void Model::_loadModel(const std::string& path, const MaterialProvider& matProvider)
{
	...
	for(unsigned int i = 0; i < m_pImportedModel->mNumMeshes; i++)
	{
		...
		volpe::VertexDeclaration* pDecl = new volpe::VertexDeclaration;
		pDecl->Begin();

        // Create the vertex buffer
		volpe::VertexBuffer* pVB = volpe::BufferManager::CreateVertexBuffer(pInterleavedMesh->pInterleavedData, pInterleavedMesh->interleavedDataSize);
        
		// Create the index buffer
		volpe::IndexBuffer* pIB = volpe::BufferManager::CreateIndexBuffer(pInterleavedMesh->pIndexData, pInterleavedMesh->numIndices);

		// We'll always have a position
		pDecl->AppendAttribute(volpe::AT_Position, 3, volpe::CT_Float, pInterleavedMesh->posOffset);

		if( pInterleavedMesh->normalOffset >= 0 )
			pDecl->AppendAttribute(volpe::AT_Normal, 3, volpe::CT_Float, pInterleavedMesh->normalOffset);

		if( pInterleavedMesh->tangentOffset >= 0 )
			pDecl->AppendAttribute(volpe::AT_Tangent, 3, volpe::CT_Float, pInterleavedMesh->tangentOffset);

		if( pInterleavedMesh->bitangentOffset >= 0 )
			pDecl->AppendAttribute(volpe::AT_BiTangent, 3, volpe::CT_Float, pInterleavedMesh->bitangentOffset);

		if( pInterleavedMesh->numColorChannels > 0 )
			pDecl->AppendAttribute(volpe::AT_Color, 4, volpe::CT_UByte, pInterleavedMesh->pColorOffsets[0]);

		for( int x = 0; x < pInterleavedMesh->numUVChannels; x++ )
			pDecl->AppendAttribute((volpe::Attribute)(volpe::AT_TexCoord1 + x), pInterleavedMesh->pUVSizes[x], volpe::CT_Float, pInterleavedMesh->pUVOffsets[x]);

        if( pInterleavedMesh->boneIndicesOffset >= 0)
        {
            pDecl->AppendAttribute(volpe::AT_BoneIndices, 1, volpe::CT_UByte4, pInterleavedMesh->boneIndicesOffset);
            pDecl->AppendAttribute(volpe::AT_BoneWeights, 4, volpe::CT_Float, pInterleavedMesh->boneWeightsOffset);
        }
...
Model::~Model()
{
	for(unsigned int i = 0; i < m_meshes.size(); i++)
	{
		...
		volpe::BufferManager::DestroyBuffer(m.m_pVB);
		volpe::BufferManager::DestroyBuffer(m.m_pIB);
		delete m.m_pDecl;
	}
	...
	for(unsigned int i = 0; i < m_meshes.size(); i++)
    {
        ...
        if(meshNode.m_ownMaterial)
            volpe::MaterialManager::DestroyMaterial(meshNode.m_pMat);
    }
}
...
void Model::Render(const glm::mat4& worldMatrix, const glm::mat4& viewMatrix, const glm::mat4& projMatrix)
{
	for(int i = 0; i < m_meshNodes.size(); i++)
	{
        ...
        volpe::Material* pMat = m.m_pMatOverride ? m.m_pMatOverride : m.m_pMat;
...
void Model::OverrideMaterial(volpe::Material *pMat)
{
	for(int i = 0; i < m_meshNodes.size(); i++)
	{
...
int Model::_getInterleavedVertexSize(const aiMesh* pSrcMesh)
{
    ...
    // Only support one color channel right now (to keep volpe simple)
...
volpe::Color4 _getColorRGBA(const aiMaterial* pMat, 
                           const char* key, 
                           int type, 
                           int idx, 
                           float defaultR = 1.0f, 
                           ...
                           float defaultA = 1.0f)
{
    ...
    return volpe::Color4(c.r, c.g, c.b, 1.0f);
}
...
volpe::Texture* _getTexture(const aiMaterial* pMat, aiTextureType texType)
{
    aiString path; 
    aiReturn ret = pMat->GetTexture(texType, 0, &path);
    if (ret == AI_SUCCESS)
        return volpe::TextureManager::CreateTexture(std::string("data/") + path.C_Str());

    return nullptr;
}

volpe::Material* Model::_createMaterialFromAssimpMaterial(const aiMaterial* pAssimpMat)
{
    aiString name;
    pAssimpMat->Get(AI_MATKEY_NAME,name);
    volpe::Material *pMat = volpe::MaterialManager::CreateMaterial(m_path + "/" + name.C_Str());
    pMat->SetProgram("data/uber.vsh", "data/uber.fsh");
    pMat->SetDepthTest(true);
    pMat->SetDepthWrite(true);

    volpe::Texture* pAlbedo = _getTexture(pAssimpMat, aiTextureType_DIFFUSE);
    if(pAlbedo)
    {
